...
// падает на разборе.
//
// Живой случай: скилл ревью/render получал в stdin результат шага
...
// `скилл ревью` объявлял `deliver: reply` у ассета-рендера, вывод
...
// assetDeliver возвращает маршрут `_deliver` для первого ассета, уходящего в
// аргументы по ссылке и объявившего доставку. "none" — осознанный отказ автора
// от доставки, он равен её отсутствию.
func (s *state) assetDeliver(args map[string]any) string {
	for _, name := range AssetRefsInArgs(args) {
		...
		switch a.Deliver {
		case AssetDeliverReply:
			return "reply"
		case AssetDeliverFile:
			return "user"
		}
	}
	return ""
