...
steps:
  - name: logs
    ...
    call:
      ...
      save_as: out
`)
	_, _, err := ExecuteWith(context.Background(), f, Deps{Caller: c}, map[string]string{"cluster": "test-k8s"})
	require.NoError(t, err)
	assert.Equal(t, "test-k8s", c.server, "вызов ушёл на вычисленный сервер")
...
func TestOnServerNarrowsModelStepTools(t *testing.T) {
	r := &fakeRunner{}
	f := parseFlow(t, `
tools: ["staging-k8s", "test-k8s", "dev-k8s"]
steps:
  - name: pick
    on_server: "{{cluster}}"
    ...
    save_as: pod
`)
	_, _, err := ExecuteWith(context.Background(), f, Deps{Runner: r}, map[string]string{"cluster": "dev-k8s"})
	require.NoError(t, err)
	require.Len(t, r.seen, 1)
	assert.Equal(t, []string{"dev-k8s"}, r.seen[0].Tools,
