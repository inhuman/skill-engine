...
func (s *state) callStep(ctx context.Context, step Step) (bool, error) {
	...
	// в событиях не было ни шага, ни причины (живой промах 29.07: два шага
	// скилл графиков молча выпали, и это выглядело как их отсутствие в описании).
...
func (s *state) forEachStep(ctx context.Context, step Step) (bool, error) {
	...
	// Прямой доступ к переменной этого не умел, и цикл по результату mcp-exec
	...
	// это как «модель ничего не нашла» (31.07).
	//
...
func (s *state) parallelStep(ctx context.Context, step Step) (bool, error) {
	...
	if p.Collect != "" {
		...
		// «в источник не ходили»: живой случай 29.07 — скилл поиска написал «в
		// Jira по теме пусто», не сделав туда ни одного запроса.
...
//
// Без этой проверки описание обошло бы собственный запрет: поток
// скилл справки осознанно убрал из набора один из серверов (за месяц 53
// попытки и ни одного успешного чтения), а прямой вызов дотянулся бы до него
// в обход. Ограничение, которое можно обойти изнутри, ничего не ограничивает.
