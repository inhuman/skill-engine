...
func testFacts() lint.Facts {
	return lint.Facts{
		ServerNames: func() []string { return []string{"docs", "tracker", "runner", "store"} },
		AllTools: func() map[string][]string {
			return map[string][]string{
				"docs":    {"page_search", "page_get"},
		...
		ToolSchemas: func() map[string][]byte {
			return map[string][]byte{
				"docs:page_get": []byte(`{"type":"object","required":["title"]}`),
			}
		},
...
func TestMissingFactsAreReportedNotSwallowed(t *testing.T) {
  ...
  steps:
    - name: fetch
      call: {tool: "docs:page_get", args: {title: x}, save_as: page}
