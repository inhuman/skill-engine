...
func TestCleanWorkflowIsSilent(t *testing.T) {
  ...
  steps:
    - name: search
      call: {tool: "wiki:page_search", args: {query: "{{input}}"}, save_as: hits}
    - name: answer
...
func TestW2_ServerNotDeclaredBySkill(t *testing.T) {
	rep := lintSkill(t, wf(`  tools: ["wiki", "tracker"]
  steps:
    - name: s
      instruction: do it
      tools: ["tracker"]
...
func TestW3_UnknownToolOnServer(t *testing.T) {
  ...
  steps:
    - name: s
      call: {tool: "wiki:page_serch", args: {query: x}, save_as: hits}
`))
...
func TestW5_MissingRequiredArg(t *testing.T) {
  ...
  steps:
    - name: s
      call: {tool: "wiki:page_get", args: {}, save_as: out}
`))
...
func TestW5_CompleteArgsAreSilent(t *testing.T) {
  ...
  steps:
    - name: s
      call: {tool: "wiki:page_get", args: {title: "{{input}}"}, save_as: out}
`))
...
func TestW12_ProtocolNamedIsQuiet(t *testing.T) {
  ...
  steps:
    - name: search
      instruction: "find it: call_tool(server=\"wiki\", tool=\"page_search\", args={})"
      tools: ["wiki"]
`))
    ...
    - name: branch
      switch:
        ...
        cases:
          a:
            - name: inner
              call: {tool: "wiki:page_serch", args: {}, save_as: out}
`},
		{"if", `  steps:
    - name: pick
      set: {var: mode, value: a}
