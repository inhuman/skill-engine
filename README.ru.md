...
Схема формата — `skill.schema.yaml` (go:embed), она же источник истины и то,
что `SchemaSummary` отдаёт модели. `skill.schema.ru.yaml` — её русский перевод:
валидировать им можно, движок его не читает; структуру двух файлов держит
одинаковой тест, так что расходятся только описания.
...
молча потеряв поля, которых в структуре больше нет. Изменения формата —
`CHANGELOG.md`.
