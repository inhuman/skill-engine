...
`), &f))
	...
	_, outcome, err := ExecuteWith(t.Context(), &f, Deps{Delegate: constDelegate("data")},
		map[string]string{"pick": `{"docs":true,"catalog":false}`})
	require.NoError(t, err)
  ...
  - name: gather
    parallel:
      ...
      branches:
        - - name: probe_docs
            when: pick.docs == true
            delegate: {skill: docs-search, task: t, save_as: from_docs}
        - - name: probe_catalog
            when: pick.catalog == true
            delegate: {skill: catalog-search, task: t, save_as: from_catalog}
`), &f))

	vars, _, err := ExecuteWith(t.Context(), &f, Deps{Delegate: constDelegate("docs data")},
		map[string]string{"pick": `{"docs":true,"catalog":false}`})
	require.NoError(t, err)

	assert.Contains(t, vars["findings"], "docs data")
	assert.Contains(t, vars["findings"+SkippedSuffix], "probe_catalog",
		"the skipped branch is invisible to the next step")
	assert.NotContains(t, vars["findings"+SkippedSuffix], "probe_docs")
}
